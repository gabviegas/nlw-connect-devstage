...
class EventsCreator:
    def __init__(self, events_repo: EventosRepositoryInterface):
        self.__events_repo = events_repo # __ = método privado
    
    def create(self, http_request: httpRequest) -> httpResponse:
        events_info = http_request.body["data"]
        event_name = events_info["name"]

        self.__check_event(event_name) # checa se há um evento com mesmo nome. se houver, levanta exceção.
        self.__insert_event(event_name) # insere um evento no banco de dados
